...
export default async function ReportPage({
  ...
  if (precedentWorkspaceIds.length) {
    ...
    for (const w of precWs ?? []) {
      precedentWorkspaceTitleById.set(String((w as any).id), String((w as any).title || "Expediente"))
    }
  }
                ...
                {Array.isArray(s.paragraphs) && s.paragraphs.length ? (
                  <div className="space-y-3">
                    {s.paragraphs.map((p: any, pIdx: number) => (
                      <p
                        key={pIdx}
                        className="whitespace-pre-wrap text-sm leading-6 text-foreground/95"
                      >
                        {p.text}
                      </p>
                    ))}
            ...
            <div className="pt-2">
              ...
              <div className="mt-2 space-y-2">
                {chronology.slice(0, 80).map((e: any, idx: number) => (
                  <div
                    key={idx}
                    className="rounded-xl border border-border/55 bg-background/25 p-3"
                  >
                    <div className="text-xs text-muted-foreground">
              ...
              <div className="mt-2 space-y-2">
                {precedents.slice(0, 10).map((p: any, idx: number) => (
                  ...
                  >
                    ...
                    {p.why ? (
                      <div className="mt-1 text-xs text-muted-foreground">{String(p.why)}</div>
                    ) : null}

                    {p.workspaceId && p.reportId && precedentWorkspaceTitleById.has(String(p.workspaceId)) ? (
                      <div className="mt-2">
                        <Link
                          href={`/workspaces/${String(p.workspaceId)}/reports/${String(p.reportId)}`}
              ...
              <div className="mt-2 space-y-2">
                {citations.map((c, idx) => (
                  ...
                  >
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        {(() => {
                          ...
                          return (
                            <>
                              snapshot_id:{" "}
                              <span className="font-code text-foreground/90">{String(c.snapshotId)}</span>
                              {fetchedAt ? (
                                <>
                                  {" "}· fetched_at:{" "}
                                  <span className="font-code text-foreground/90">{String(fetchedAt)}</span>
                                </>
                              ) : null}
                              {hash ? (
                                <>
                                  {" "}· hash:{" "}
                                  <span className="font-code text-foreground/90">{String(hash)}</span>
                                </>
                              ) : null}
                            </>
                          )
                    ...
                    <div className="mt-2 whitespace-pre-wrap text-sm">{c.quote}</div>
                  </div>
                ))}
              </div>
            </div>
