...
export default async function ProjectOpsPage({
  params,
}: {
        ...
        <Card className="bg-card/70 lg:col-span-2">
          ...
          <CardContent className="space-y-3">
            ...
            {list.length ? (
                    ...
                    {list.slice(0, 80).map((j) => (
                      <tr key={j.id} className="border-t border-border/55 align-top">
                        <td className="px-3 py-2">
                          <Badge variant="outline" className={statusBadgeClass(String(j.status || ""))}>
                            {String(j.status || "-")}
                          </Badge>
                        </td>
                        ...
                        <td className="px-3 py-2">
                          {j.last_error ? (
                            ...
                            <div className="text-xs text-muted-foreground">
                              ...
                              {j.available_at
                                ? `Disponible: ${new Date(j.available_at).toLocaleString("es-CL")}`
                                : ""}
                            </div>
                          )}
                        </td>
              ...
              <div className="text-sm text-muted-foreground">Sin jobs para este proyecto.</div>
            )}
          </CardContent>
        </Card>
      </div>
