...
export default async function ExcelWatchlistPage({
  params,
}: {
          ...
          <CardContent className="space-y-2">
            {runs?.length ? (
              runs.map((r) => (
                <Link
                  key={r.id}
                  href={`/excel/${workspaceId}/${watchlistId}/runs/${r.id}`}
            ...
            {emails?.length ? (
              emails.map((e) => (
                <div key={e.id} className="rounded-xl border border-border/55 bg-background/25 px-3 py-2">
                  ...
                  {e.error ? <div className="mt-2 text-xs text-destructive">{e.error}</div> : null}
                </div>
              ))
            ) : (
